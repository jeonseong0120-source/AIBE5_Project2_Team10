...
import { User as UserIcon, Code2, Briefcase, Star, Award, Settings, Save, MapPin, Activity, Plus, Trash2, X, Clock, Image as ImageIcon, Upload, Loader2, Calendar, ChevronLeft, ChevronRight, Edit3, Send, Inbox } from 'lucide-react';
                        ...
                        >
                            ...
                            <div className="w-full md:w-[55%] bg-zinc-50 flex flex-col relative group shrink-0">
                                <div className="flex-1 relative flex items-center justify-center p-8 min-h-[300px]">
                                    <img src={selectedPortfolio.portfolioImages?.[activeImageIndex] || selectedPortfolio.thumbnailUrl || "https://placehold.co/600x400?text=No+Image"} alt="main_portfolio" className="max-w-full max-h-full object-contain drop-shadow-sm transition-opacity duration-300" />
                                    
                                    {/* 슬라이더 컨트롤 */}
                                    {selectedPortfolio.portfolioImages && selectedPortfolio.portfolioImages.length > 1 && (
                                        <>
                                            <button 
                                                onClick={(e) => { e.stopPropagation(); setActiveImageIndex(prev => prev === 0 ? selectedPortfolio.portfolioImages.length - 1 : prev - 1); }} 
                                            ...
                                            <button 
                                                onClick={(e) => { e.stopPropagation(); setActiveImageIndex(prev => prev === selectedPortfolio.portfolioImages.length - 1 ? 0 : prev + 1); }} 
